...
# Class generated via LLM chatgpt-4
class SpherePicker:
    def __init__(self, points, labels):
        self.points = points
        self.labels = labels

        self.picker = tvtk.CellPicker()
        self.picker.add_observer("EndPickEvent", self.on_pick)
        self.labels_visibility = [False] * len(points)

    def on_pick(self, picker):
        point_id = picker.point_id
        if point_id >= 0:
            if self.labels_visibility[point_id]:
                self.labels[point_id].visibility = False
            else:
                self.labels[point_id].visibility = True

            self.labels_visibility[point_id] = not self.labels_visibility[point_id]
...
def visualize_umap_3d(umap_embeddings):
    ...
    for i in range(len(x)):
        sphere = mlab.points3d(x[i], y[i], z[i], color=colors[i], scale_factor=0.1)
        label = mlab.text3d(x[i], y[i], z[i] + 0.1, df['input_name'].iloc[i], scale=0.05, color=(0, 0, 0))
        label.visibility = False

        spheres.append(sphere)
        labels.append(label)

    picker = SpherePicker(spheres, labels)
    mlab.get_engine().iren.add_observer("LeftButtonPressEvent", picker.on_pick)
